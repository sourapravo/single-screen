...
document.addEventListener("DOMContentLoaded", () => {
  ...
  $$(".side-toggle").forEach(button => {
    ...
    // Absolutely closed on every fresh page load.
  ...
  function setActive(item) {
    sidebarItems.forEach(link => link.classList.remove("is-active"));
    if (!item) return;

    item.link.classList.add("is-active");

    // If a subsection is active, visually retain its parent as active too,
    // but do NOT automatically expand it. The user controls expansion.
    const parentGroup = item.link.closest(".side-group");
    if (parentGroup) {
      const parentLink = $(".side-parent-link", parentGroup);
      if (parentLink) parentLink.classList.add("is-parent-context");
    }
  }
