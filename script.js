...
/* FINAL GRID LAYOUT OVERRIDE: side-by-side, equal height, natural width */
(function(){
  function fitGrid(grid){
    const imgs=[...grid.querySelectorAll("img")].filter(i=>i.naturalWidth&&i.naturalHeight);
    if(!imgs.length) return;
    const gap=parseFloat(getComputedStyle(grid).gap)||0;
    const available=grid.clientWidth-gap*(imgs.length-1);
    const ratios=imgs.map(i=>i.naturalWidth/i.naturalHeight);
    const sum=ratios.reduce((a,b)=>a+b,0);
    if(sum<=0) return;
    const h=Math.min(190,available/sum);
    imgs.forEach((img,i)=>{
      img.style.width=(h*ratios[i])+"px";
  ...
  document.querySelectorAll(".image-grid").forEach(grid=>{
    ...
    grid.querySelectorAll("img").forEach(img=>{
      if(img.complete) fitGrid(grid);
      else img.addEventListener("load",()=>fitGrid(grid),{once:true});
    });
  });
  window.addEventListener("resize",()=>document.querySelectorAll(".image-grid").forEach(fitGrid));
  window.addEventListener("load",()=>document.querySelectorAll(".image-grid").forEach(fitGrid));
})();


/* ============================================================
   FINAL V3 GRID FIT — larger equal-height grids, body-width bound
   ============================================================ */
(function(){
  function fitFinalGrid(grid){
    const imgs=[...grid.querySelectorAll('img')].filter(i=>i.naturalWidth&&i.naturalHeight);
    if(!imgs.length) return;
    const gap=parseFloat(getComputedStyle(grid).gap)||0;
    const ratios=imgs.map(i=>i.naturalWidth/i.naturalHeight);
    ...
    const maxH=window.innerWidth<=760 ? 260 : 330;
    const h=Math.min(maxH, available/sum);
    imgs.forEach(img=>{
      img.style.height=h+'px';
  ...
  function fitAll(){document.querySelectorAll('.image-grid').forEach(fitFinalGrid)}
  document.querySelectorAll('.image-grid img').forEach(img=>{
    if(img.complete) fitFinalGrid(img.closest('.image-grid'));
    else img.addEventListener('load',()=>fitFinalGrid(img.closest('.image-grid')),{once:true});
  });
